...
import { BrainCircuit, Code2, Landmark, Lightbulb, Rocket, Shield } from 'lucide-react';
...
export const metadata: Metadata = {
  ...
  description: 'Meu Labs offers coding, STEM, AI, software, university access and founder programmes for children and teens in New Zealand.',
  openGraph: { images: ['/og-default.jpg'] }
};

const featuredSlugs = ['kx', 'coding-software', 'se', 'ua'];
...
const nzPathway = [
  {
    title: 'Foundations',
    age: 'Age 8 - 12',
    body: 'Build strong foundational skills across coding, design, communication, and leadership while developing essential 21st-century skills.',
    courses: [
      { name: 'STEM For Kids: Junior', icon: Rocket, href: '/nz/courses/kx-j' },
      { name: 'STEM For Kids', icon: Shield, href: '/nz/courses/kx' }
    ],
  ...
  },
  {
    ...
    age: 'Age 10 - 14',
    body: 'Choose themed courses aligned with coding and software. Deeper, project-driven, and skill-focused.',
    courses: [
      { name: 'Coding and Software', icon: Code2, href: '/nz/courses/coding-software' }
    ],
  ...
  },
  {
    ...
    courses: [
      { name: 'Software Engineering', icon: Code2, href: '/nz/courses/se' },
      { name: 'Data Science and AI', icon: BrainCircuit, href: '/nz/courses/ds' },
      { name: 'Game Development', icon: Code2, href: '/nz/courses/gd', badge: 'COMING SOON' },
      { name: 'Cyber Security', icon: Shield, href: '/nz/courses/cs', badge: 'COMING SOON' }
    ],
  ...
  },
  {
    ...
    courses: [
      { name: 'University Access', icon: Landmark, href: '/nz/courses/ua', badge: 'COMING SOON' },
      { name: 'Founder Studio', icon: Lightbulb, href: '/nz/courses/fs' }
    ],
...
export default function NzHomePage() {
  const nzCourses = getNzCourses();
  const featuredCourses = featuredSlugs
    .map((slug) => nzCourses.find((course) => course.slug === slug))
    ...
    <main id="main-content">
        ...
        <div className="relative mx-auto grid min-h-[760px] max-w-[92rem] items-center gap-8 py-20 lg:grid-cols-[0.84fr_1.16fr]">
          <div className="max-w-4xl">
            ...
            <Reveal animation="pop" delay={300} className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ButtonLink href="/nz/courses">Explore Courses</ButtonLink>
              <ButtonLink href="/nz/contact" variant="secondary">Speak with a Student Counselor</ButtonLink>
            </Reveal>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {featuredCourses.map((course, index) => (
              <Reveal key={course.slug} animation="pop" delay={index * 90} className="h-full">
                <CourseCard course={course} basePath="/nz" />
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section className="bg-creamAlt px-4 py-16 sm:px-6 lg:px-8" aria-labelledby="learning-pathway-title">
        <div className="mx-auto max-w-[92rem]">
          ...
          <Reveal animation="pop" delay={400} className="mt-10"><ButtonLink href="/nz/courses">Explore Courses</ButtonLink></Reveal>
        </div>
      </section>
          ...
          <Reveal>
            <div className="mb-10 flex flex-col gap-8">
              <div className="w-full">
                ...
                <p className="mt-5 w-full text-lg font-extrabold leading-8 text-slate-600">From WebApps to media productions to AI agents. At Meu Labs students build real projects with real-world skills.</p>
              </div>
              <ButtonLink href="/nz/projects" className="w-fit shrink-0 self-start lg:self-end">View Student Projects</ButtonLink>
            </div>
          </Reveal>
        </div>
