...
export function SectionHeader({ eyebrow, title, subtitle, center, eyebrowStyle = 'default' }: SectionHeaderProps) {
  return (
    <div className={`mb-10 max-w-[58rem] ${center ? 'mx-auto text-center' : ''}`}>
      {eyebrow && (
      ...
      <h2 className="text-balance text-[2.85rem] font-normal leading-[1.05] text-navy md:text-[4rem]">{title}</h2>
      {subtitle && <p className="mt-5 max-w-5xl text-pretty text-lg font-extrabold leading-8 text-slate-600">{subtitle}</p>}
    </div>
  );
}
